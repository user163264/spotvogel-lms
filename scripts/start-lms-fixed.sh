...
# First, aggressively kill any processes on our required ports
echo -e "${YELLOW}Ensuring ports are available...${NC}"
"$SCRIPTS_DIR/kill-port-processes.sh"

# Verify ports are really free
is_port_in_use() {
    local port=$1
...
# Double-check ports are free after aggressive cleanup
if is_port_in_use $BACKEND_PORT; then
    echo -e "${RED}CRITICAL: Port $BACKEND_PORT is still in use after aggressive cleanup.${NC}"
...
# Start frontend server with the standard port
...
if [ -d "$FRONTEND_DIR" ]; then
    ...
    if [ -f "package.json" ]; then
        ...
        # We're NOT using the PORT environment variable here since we want it to use the default 3000
...
cleanup() {
    ...
    # Force kill any remaining processes by port using our aggressive cleanup
    echo -e "${YELLOW}Ensuring all processes are terminated...${NC}"
    "$SCRIPTS_DIR/kill-port-processes.sh"
    
    # Clean up PID files
    rm -f "$LMS_DIR/.pid.frontend" "$LMS_DIR/.pid.backend"
